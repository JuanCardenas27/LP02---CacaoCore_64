...
src_path = 'src/examples/high_level/float_insertion_sort.txt'
source = open(src_path, encoding='utf-8').read()
# Rename loop variable to a mixed-case identifier (whole-token replacement).
source = re.sub(r'\bI\b', 'indiceI', source)

pre = Preprocesador()
res, imports = pre.preprocess(source, nombre_fuente=src_path)
errs, asmc = compiler.compile_generator(res.text, imports.lista)
print('errors', errs)
open('resultado_float_insertion_sort_indiceI.asm', 'w', encoding='utf-8').write('\n'.join(asmc))
